...
import vertex from '../../src/shaders/galary/vertex.glsl'
import fragment from '../../src/shaders/galary/fragment.glsl'
...
function init () {
  ...
  const generateGalaxy = () => {
    ...
    for (let i = 0; i < galaxyParameters.count; i++) {
      ...
      const randomY = Math.pow(Math.random(), galaxyParameters.randomnessPower) * (Math.random() < 0.5 ? 0.5 : -0.5)
      ...
      // 使用 Color 类中的lerp方法将内部颜色与外部颜色合并，并根据粒子所在半径大小设置融合程度（0-1）
    ...
    material = new THREE.ShaderMaterial({
      ...
      uniforms: {
        uTime: { value: 0 },
        uSize: { value: galaxyParameters.options.size * renderer.getPixelRatio() }
      }
    })
  ...
  gui.add(galaxyParameters.options, 'size').min(1).max(100).step(0.1).onFinishChange(generateGalaxy)
